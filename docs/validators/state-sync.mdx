...
sed -i.orig -e "s|^enable = .*|enable = \"true\"|" ~/.zetacored/config/config.toml &&
...
sed -i -e "s|^external_address = .*|external_address = \"${EXTERNAL_IP}:26656\"|" ~/.zetacored/config/config.toml &&
