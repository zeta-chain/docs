...
and additionally used to secure the PoS ZetaChain blockchain by
...
- Unlike the more common two-way pegging, there is no wrapping and therefore no
  multiple representation of the same underlying asset.
- The only (native) value that can go cross-chain is via the ZETA coin, which
  ...
  audit and therefore higher security. For example, we can check total supply at
