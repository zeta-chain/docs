import { AlertTriangle, Book, ChevronRight, Github, Hash, MessageSquare, Search } from "lucide-react";
...
import { Page, PageSection, PageType, useDocsSearch } from "./useDocsSearch";
...
const DocsSearch = () => {
  ...
  const IconContainer = (props: React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>) => (
    <div
      className="
        transition
        w-6 h-6
    ...
    <>
                  ...
                  <div className="flex flex-col gap-0">
                    ...
                    {(page.description || page.subtitle) && (
                      <div className="text-xs text-foreground-muted">
                        <TextHighlighter text={page.description! || page.subtitle!} query={search} />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="border-l border-default ml-3 pt-3">
                  {page.sections.map((section, i) => (
                    <CommandItem
                      ...
                      value={`${encodeURIComponent(page.title)}__${encodeURIComponent(
                        section.heading
                      )}-item-index-${i}`}
                      forceMount={true}
                      type="block-link"
      ...
      {state.status === "error" && (
        <div className="p-6 flex flex-col items-center gap-6 mt-4">
          <AlertTriangle strokeWidth={1.5} size={40} />
          <p className="text-lg text-center">Sorry, looks like we&apos;re having some issues with search!</p>
