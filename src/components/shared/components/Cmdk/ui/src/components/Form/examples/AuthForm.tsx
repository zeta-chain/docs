...
const CardForm = () => {
  ...
  const Section = ({ children, header }: { children: React.ReactNode; header?: React.ReactNode }) => (
      ...
      <Form
        ...
        validate={(values) => {
          ...
          if (values.jwt_expiry > 604800) {
            errors.jwt_expiry = "The maxiumum allowed is 604,800 seconds. Use a smaller number of seconds.";
          }
          return errors;
        }}
      >
        {({ isSubmitting, handleReset }: any) => (
            ...
            footer={
              <>
                ...
                <div className="flex justify-end gap-2 py-3 px-6">
                  ...
                  <Button loading={isSubmitting} type="primary" htmlType="submit" form="general_form">
                    Save
                  </Button>
                </div>
              </>
          ...
          >
            <Section header={<label className="text-foreground col-span-4 text-sm">User Signups</label>}>
              <Toggle
                id="enable_signups"
            ...
            <div className="border-muted border-t"></div>
            <Section header={<label className="text-foreground col-span-4 text-sm">User Sessions</label>}>
              <InputNumber
                className="col-span-8"
                id="jwt_expiry"
...
const AuthForm = () => {
  ...
  const CollapsibleContainer = ({ children, header }: { children: React.ReactNode; header?: React.ReactNode }) => {
    return (
      <div
        className="
  ...
  const ProviderForm = ({ provider }: { provider: Provider }) => {
    ...
    const [active, setActive] = useState(providersFormState[`${provider.name.toLowerCase()}-active`]);
        ...
        <Form
          ...
          initialValues={{
            [`${provider.name.toLowerCase()}-active`]:
              ...
              true,
            [`${provider.name.toLowerCase()}-app-id`]: providersFormState[`${provider.name.toLowerCase()}-app-id`],
            [`${provider.name.toLowerCase()}-id-key`]: providersFormState[`${provider.name.toLowerCase()}-id-key`],
          }}
          validate={(values) => {
            const errors: any = {};
            ...
            if (values[input_active]) {
              if (!values[input_app_id])
                errors[input_app_id] = "You need to input an application ID if you want to enable this provider.";
              if (!values[input_id_key])
                errors[input_id_key] = "You need to input an application secret if you want to enable this provider.";
            }
            ...
            <Form
              ...
              validate={(values) => {
                ...
                if (!values.domain) {
                  ...
                  !/^((https?|ftp|smtp):\/\/)?(www.)?[a-z0-9]+\.[a-z]+(\/[a-zA-Z0-9#]+\/?)*$/i.test(values.domain)
                ) {
                  ...
                  <div className="mb-4 space-y-4 pt-4">
                    <div className="px-5">
                      <p className="text-foreground-light text-sm">
                        This will add a domain to a list of allowed domains that can interact with your Authentication
                        services for this project.
                      </p>
                    </div>
                    <div className="border-overlay-border border-t"></div>
                    <div className="px-5">
                      <Input id="domain" name="domain" label="Domain" placeholder="https://mydomain.com" />
                    </div>
                    <div className="border-overlay-border border-t"></div>
                    <div className="px-5">
                      <Button form="new-domain-form" htmlType="submit" block size="medium" loading={isSubmitting}>
                        Add domain
                      </Button>
                    </div>
                  </div>
        ...
        <div className="mb-4 space-y-4 pt-4">
          <div className="px-5">
            <p className="text-foreground-light mb-2 text-sm">
              Are you sure you want to remove <span className="text-foreground">{selectedDomain}</span>?
            </p>
            <p className="text-foreground-muted text-sm">
          ...
          <Panel
            footer={
              <>
                <div className="border-muted border-t"></div>
                <div className="flex justify-end gap-2 py-3 px-6">
                  <Button loading={isSubmitting} type="secondary" htmlType="reset" onClick={() => handleReset()}>
                    Cancel
                  </Button>
                  <Button loading={isSubmitting} type="primary" htmlType="submit" form="general_form">
                    Save
          ...
          >
            ...
            >
              <div className="col-span-8 space-y-6">
                ...
                <Input label="SMTP account password" type="password" disabled={!values.enable_smtp} />
              </div>
            </Section>
            {values.enable_smtp && (
              <div className="mx-8">
              ...
              <Section
                header={
                  <div className="w-full">
                    ...
                    <p className="text-foreground-muted text-sm">
                      When a user signs up using an email address and password, you can send them a confirmation email
                      to verify their registered email address. Learn more
                    </p>
                  </div>
                }
              >
