 ...
 */
const Input = ({ className = "", type = "text", size = "medium", isError = false, children, ...props }) => {
  const classes = [];
  if (size) {
    const sizeClasses = {
  ...
  return <input type={type} className={`form-input border-solid ${classes.join(" ")} ${className}`} {...props} />;
};
