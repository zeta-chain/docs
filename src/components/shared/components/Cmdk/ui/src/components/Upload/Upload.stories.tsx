...
export const Draggable = () => {
  ...
  return (
    <Upload.Dragger label="Upload CSV" layout="horizontal" files={files} setFiles={setFiles}>
      <div className="flex flex-col gap-8 justify-evenly">
        ...
        {files.length > 0 && (
          <div className="grid grid-flow-row grid-cols-3 gap-8 items-center justify-evenly m-auto">
            {files.map((file) => (
              <img className="h-32 object-cover w-full rounded-lg" src={URL.createObjectURL(file)} alt={file.name} />
            ))}
          </div>
        )}
      </div>
