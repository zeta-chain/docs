...
function Listbox({
  ...
  const { formContextOnChange, values, errors, handleBlur, touched, fieldLevelValidation } = useFormContext();
      ...
      <DropdownMenuPrimitive.Root>
        <DropdownMenuPrimitive.Trigger asChild disabled={disabled}>
          ...
          >
            ...
            {error && <div className={__styles.actions_container}>{error && <InputErrorIcon size={size} />}</div>}
          </button>
        ...
        >
          <div>
            <SelectContext.Provider value={{ onChange: handleOnChange, selected }}>{children}</SelectContext.Provider>
          </div>
        </DropdownMenuPrimitive.Content>
      </DropdownMenuPrimitive.Root>
    </FormLayout>
 ...
 * @deprecated Use ./Select_shadcn_ or follow ComboBox convention or use ./ui-patterns/multi-select
 */
function SelectOption({ id, value, label, disabled = false, children, className = "", addOnBefore }: OptionProps) {
  const __styles = styleHandler("listbox");
        ...
        return (
          ...
          >
            <div className={__styles.option_inner}>
              ...
              <span>{typeof children === "function" ? children({ active, selected }) : children}</span>
            </div>

            {active ? (
              <span className={cn(__styles.option_check, active ? __styles.option_check_active : "")}>
