    ...
    <>
      {visible && (
        <div className={containerClasses.join(" ")}>
          {withIcon ? <div className={__styles.variant[variant].icon}>{withIcon && icons[variant]}</div> : null}
          {icon && icon}
          <div className="flex flex-1 items-center justify-between">
            <div>
              <h3 className={[__styles.variant[variant].header, __styles.header].join(" ")}>{title}</h3>
              <div className={descriptionClasses.join(" ")}>{children}</div>
            </div>
            {actions}
          </div>
          {closable && (
            <button aria-label="Close alert" onClick={() => setVisible(false)} className={closeButtonClasses.join(" ")}>
