...
>(({ className, padding, ...props }, ref) => (
  <div
    ref={ref}
    {...props}
    className={cn("flex flex-col gap-1.5 text-center sm:text-left", DialogPaddingVariants({ padding }), className)}
  />
));
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title ref={ref} className={cn("text-base leading-none font-normal", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description ref={ref} className={cn("text-sm text-foreground-lighter", className)} {...props} />
));
...
const DialogSectionSeparator = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => (
