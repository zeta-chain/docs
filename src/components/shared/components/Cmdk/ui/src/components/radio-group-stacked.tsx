...
>(({ className, ...props }, ref) => {
  return (
    <RadioGroupPrimitive.Root className={cn("flex flex-col -space-y-px w-full", className)} {...props} ref={ref} />
  );
});
    ...
    >
      <div className="flex gap-3 w-full px-[21px] py-[16px]">
        ...
        <div className="flex flex-col gap-1">
          ...
          {props.description && <p className="text-xs text-foreground-lighter">{props.description}</p>}
          {props.children}
        </div>
      </div>
    </RadioGroupPrimitive.Item>
