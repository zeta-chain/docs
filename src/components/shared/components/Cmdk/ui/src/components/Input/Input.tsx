 ...
 */
export interface TextAreaProps extends Omit<React.InputHTMLAttributes<HTMLTextAreaElement>, "size" | "onCopy"> {
        ...
        {copy || error || actions ? (
          <div className={__styles["textarea_actions_container"]}>
            <div className={__styles["textarea_actions_container_items"]}>
              {error && <InputErrorIcon size={size} />}
              {copy && (
                <Button size="tiny" type="default" onClick={() => _onCopy(value)} icon={<IconCopy />}>
                  {copyLabel}
                </Button>
              )}
              {actions && actions}
