...
export function FormLayout({
  ...
  const renderError = (
    <p data-state={error ? "show" : "hide"} className={[__styles.error.base, __styles.error.size[size]].join(" ")}>
      {error}
    </p>
  );

  const renderDescription = descriptionText && (
    <div className={[__styles.description.base, __styles.description.size[size]].join(" ")} id={id + "-description"}>
      {descriptionText}
    </div>
  );
  ...
  return (
    <div className={containerClasses.join(" ")}>
      ...
      {labelled || labelOptional || layout === "horizontal" ? (
        ...
        >
          {labelled && (
            <label className={[__styles.label.base, __styles.label.size[size]].join(" ")} htmlFor={id}>
              {beforeLabel && (
                <span
