...
function Overlay({
  ...
  let classes = [OverlayStyles["sbui-overlay-container"], OverlayStyles[`sbui-overlay-container--${placement}`]];
    ...
    <div ref={ref} className={OverlayStyles["sbui-overlay"]}>
      ...
      >
        <div className={classes.join(" ")} style={overlayStyle}>
          <DropdownContext.Provider value={{ onClick: onToggle }}>{children}</DropdownContext.Provider>
        </div>
      </Transition>
      {placement === "topRight" || placement === "topLeft" || placement === "topCenter" ? <TriggerElement /> : null}
    </div>
  );
}
