      ...
      if (!allResults.length) {
        return allSourcesLoaded
          ...
          : {
              status: "loading",
              key: action.key,
              staleResults: "results" in state ? state.results : "staleResults" in state ? state.staleResults : [],
            };
      }
      return allSourcesLoaded
        ? {
...
const useDocsSearch = () => {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    // supabaseClient
    //   .rpc("match_page_sections", { embedding: "", match_threshold: 0.7, match_count: 10, min_content_length: 50 })
    //   .then(({ data, error }) => {
    //     sourcesLoaded += 1;
