...
import { NodeSnapshots } from "~/components/Docs/components/NodeSnapshots";
...
Choose the network and snapshot type, download the snapshot and extract the
snapshot inside `~/.zetacored` directory.

<NodeSnapshots apiUrl="https://snapshots.rpc.zetachain.com" />

<Alert variant="warning">
  {" "}
  Note: Our nodes database backend is `pebbledb`. Ensure that your node is configured to use the same backend to utilize
  our snapshots.{" "}
</Alert>
