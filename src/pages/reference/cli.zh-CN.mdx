...
一个用于构建与交互 [ZetaChain](https://www.zetachain.com) 全链应用的命令行工具。通过同一个 CLI 即可无缝操作 EVM、Solana、Bitcoin、Sui 与 TON。

## ✨ 功能

- 基于模板快速搭建新的 ZetaChain 全链应用
- 一条命令启动涵盖 EVM、Solana 等多链的本地开发环境
- 查询跨链手续费、合约、余额、跨链交易、代币等信息
- 在 Solana、Sui、Bitcoin、TON 与 ZetaChain 全链应用之间发起跨链调用
- 在已连接的链之间转移受支持的代币
...
欢迎贡献代码或反馈问题，随时提交 Issue 或 Pull Request。
