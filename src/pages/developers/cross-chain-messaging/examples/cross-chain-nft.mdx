...
<Image
  src="/img/graphs/cross-chain-nft-transfer.svg"
  alt="Cross-chain NFT transfer"
  width={811}
  height={387}
  className="w-full"
/>
...
```solidity filename="contracts/CrossChainWarriors.sol" {8-9,14,21-22,32,37-38,41-57,73,83,101,115}
...
<CodeBlock
  language="ts"
  filename="tasks/mint.ts"
  codeUrl="https://raw.githubusercontent.com/zeta-chain/example-contracts/main/messaging/warriors/tasks/mint.ts"
/>
...
<CodeBlock language='ts' filename="tasks/interact.ts" code={
`const paramSender = hre.ethers.utils.getAddress(args.sender); // [!code --]

const tx = await contract
.connect(signer)
.sendMessage(destination, paramToken, paramTo, { value }); // [!code highlight]
...
```ts filename="tasks/deploy.ts" {5-8,17,26}
const main = async (args: any, hre: HardhatRuntimeEnvironment) => {
  ...
  await Promise.all(
    networks.map(async (networkName: string, i: number) => {
      const parity = i % 2 == 0;
      contracts[networkName] = await deployContract(hre, networkName, parity, args.json, args.gasLimit);
    })
  );
