...
<Image
  src="/img/graphs/cross-chain-counter.svg"
  alt="Cross-chain counter"
  width={811}
  height={387}
  className="w-full"
/>
...
```solidity filename="contracts/Counter.sol" {11,15,31,46,63,77-78}
...
```ts filename="tasks/counter_show.ts"
...
const paramFrom = hre.ethers.utils.getAddress(args.from); // [!code --]

const value = parseEther(args.amount);

const tx = await contract
.connect(signer)
.sendMessage(destination, { value }); // [!code highlight]
