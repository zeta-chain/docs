...
The depositAndCall `op_code` is `102`. `query_id` is reserved for future use, leave it to `0`. Also note that call_data should be a cell encoded in ["snake data"](https://docs.ton.org/v3/guidelines/dapps/asset-processing/nft-processing/metadata-parsing#snake-data-encoding) format (supported by most TON libraries)
