...
<Alert>Gateway is an upcoming unified interface that will replace the TSS address and ERC-20 custody contract.</Alert>
...
Each native gas and supported ERC-20 token from a connected chains has a
...
span multiple blockchains can been triggered by a single user-signed
transaction.
...
- Native gas and supported ERC-20 tokens are represented as ZRC-20 tokens. A
  ZRC-20 token can be permissionlessly withdrawn back to it's original chain.
