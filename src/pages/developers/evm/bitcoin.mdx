...
6. If the purpose of this Bitcoin transaction is to deposit BTC and also use the
   ...
   [ZetaChain's EVM contract address (20B)] + [arbitrary binary message]`

All inputs and outputs of Bitcoin transactions sent to TSS address must be one
of the following supported addresses: P2WPKH (SegWit bech32 address, such as
bc1xxx on Bitcoin mainnet), P2TR, P2PKH, P2SH, P2WSH. Otherwise the transactions
will not be observed and there will be no refund, and may cause irreversible
user fund loss.
...
<Alert variant="tip"> {" "} In order to test with Bitcoin, you will need to use
  a wallet that allows setting an `OP_RETURN` and memos in a binary format.
  Please see our wallet suggestions [here](/reference/wallets).{" "} </Alert>
