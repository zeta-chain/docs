ZetaChain uses two mechanisms to manage liquidity throughput and ensure smooth
...
network. This not only prevents abuse but also safeguards the network from
potential liquidity shocks and ensures that the system can handle high
transaction loads without compromising performance or security.
...
can be accessed through the the chain's API: on
...
limiter mechanism. This mechanism ensures that the total amount of tokens
