...
```bash
UNIVERSAL=$(tsx commands/index.ts deploy --private-key $PRIVATE_KEY | jq -r .contractAddress) && echo $UNIVERSAL
```
...
```bash
RECIPIENT=$(cast wallet address $PRIVATE_KEY) && echo $SENDER
```
...
```bash
zetachain query cctx --hash 28xsic7NqafyxqDjmqfYL5f6RoHFYLrCKvjSA4UJCXyESmdCb1bVpW3dqT2QJrwV6KmfdWuHrwj8uW4txHZiXLxm
```
