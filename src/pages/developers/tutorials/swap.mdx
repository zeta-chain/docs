...
<Alert>
  ...
  that works with the current testnet, checkout the [Swap on Testnet tutorial](/developers/tutorials/swap-tss){" "}
</Alert>
...
approves each token separately—the gas fee token (`gasZRC20`) and the target
token (`params.target`).
