...
The `Universal` contract is a simple universal app deployed on ZetaChain. It
...
Here's the code for the `Universal` contract:
...
The `Connected` contract is a simple contract deployed on an EVM-compatible
chain. It can be invoked by the `Universal` contract on ZetaChain to demonstrate
...
- `--contract`: Address of the `Connected` contract on the connected EVM chain.
- `--receiver`: Address of the `Universal` contract on ZetaChain.
...
- **EVM Chain**: Executes the `call` function of the `Connected` contract.
- **EVM Chain**: The `call` function invokes `gateway.call`, emitting a `Called`
  event.
- **ZetaChain**: The protocol detects the event and triggers the `Universal`
  contract's `onCall`.
- **ZetaChain**: The `Universal` contract decodes the message and emits a
  `HelloEvent`.
...
- `--contract`: Address of the `Universal` contract on ZetaChain.
- `--receiver`: Address of the `Connected` contract on the connected EVM chain.
- `--zrc20`: Address of the ZRC-20 token representing the gas token of the
  connected chain. This determines the destination chain.
- `--function`: Function signature to invoke on the `Connected` contract (e.g.,
  `"hello(string)"`).
- `--network`: Network to interact with (`localhost` for localnet).
...
- `--function`: Function signature to invoke on the `Connected` contract.
