...
contracts deployed on ZetaChain, and withdraw tokens back to Solana — even
...
Pass `sol` to encode your accounts/data to pass to your Solana program:

```
USDC_SPL=FWoDc17eRYqQUYG6RwYgC7ZDBtKNaUvvNqwRYi1rD9US
ENCODED_ACCOUNTS_AND_DATA=$(npx ts-node solana/setup/encodeCallArgs.ts "sol" "$USDC_SPL")
```

Use ZRC-20 SOL address as `--zrc20`:
...
Pass `spl` to encode your accounts/data to pass to your Solana program:
...
Use ZRC-20 USDC address as `--zrc20`:
